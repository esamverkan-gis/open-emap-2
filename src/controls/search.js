...
import wktToFeature from '../maputils';
...
function selectHandler(evt) {
  ...
  if (layerNameAttribute && idAttribute) {
    ...
    if (data[layerNameAttribute] === 'Fastighet') {
      ...
      var areaPromise = fetchFastighetsYta(objectId);
      areaPromise.then(function (response) {
        ...
        */
        if (features.length > 1) {
          console.log("Found FeatureCollection with multiple features. Tryging to merge them into a Multigeometry");
          if (features[0].getGeometry().getType() == "Polygon") {
            let multiGeom = new MultiPolygon();
...
function initAutocomplete() {
  ...
  function makeRequest2(handler, obj) {
    ...
    prepSuggestions.makeRequest(prepOptions, obj.value).then(function (response) {
      // IE cannot handle this! use flattenData function instead.
      // var data = [...response[0], ...response[1], ...response[2]];
      var data = flattenData(response);
  ...
  $(input).on('keyup', function func(e) {
    ...
    if (this.value.length >= minLength) {
      if (keyCode in keyCodes) {
        ...
        delay(function () { makeRequest2(handler, that); }, 500);
      }
    }
  });
}
...
function fetchFastighetsYta(objectId) {
  ...
  var url = url_yta.replace("objectId", objectId);
...
function init(options) {
  ...
  prepOptions.url_fastighet = options.url_fastighet;
  prepOptions.url_adress = options.url_adress;
  prepOptions.url_ort = options.url_ort;
  prepOptions.municipalities = options.municipalities;
  url_yta = options.url_yta;
